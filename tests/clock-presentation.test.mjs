...
test("mantém a saída e identifica entrada pendente do dia anterior em São Paulo", () => {
  ...
  assert.deepEqual(result, {
    nextType: "CLOCK_OUT", lastEntryTime: "23:30:00",
    lastEntryDate: "14/09/2026", hasPendingPreviousDay: true,
  });
});
...
test("usuário sem movimentos pode registrar entrada sem inventar horário", () => {
  const result = buildClockPresentation({ nextEntryType: "CLOCK_IN", lastEntry: null }, DateTime.fromISO("2026-09-15T12:00:00Z"));
  assert.deepEqual(result, {
    nextType: "CLOCK_IN", lastEntryTime: null,
    lastEntryDate: null, hasPendingPreviousDay: false,
  });
});
