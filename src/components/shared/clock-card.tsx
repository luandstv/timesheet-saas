...
export function ClockCard({
  ...
  async function handleClick() {
    ...
    try {
      ...
      setError(
        "Não foi possível confirmar o registro. Confira seus pontos antes de tentar novamente.",
      );
    } finally {
      ...
      <CardContent className="grid flex-1 gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.9fr)]">
        <div className="flex min-w-0 flex-col">
          <div className="flex items-center gap-3">
            <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-accent-foreground">
              <AlarmClock className="size-6" />
            </span>
            <div>
              <h2 className="font-semibold">Agora</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                Horário oficial do seu ponto
              </p>
            </div>
          </div>
          <div className="flex flex-1 flex-col items-center justify-center pt-5">
            ...
            <p className="mt-3 text-center text-xs leading-5 text-muted-foreground">
              {lastEntryTime
                ? `${isWorking ? "Entrada" : "Saída"}${lastEntryDate ? ` de ${lastEntryDate}` : ""} às ${lastEntryTime}`
                : "Nenhum movimento registrado hoje"}
            </p>
            {hasPendingPreviousDay && (
              <p
                role="status"
                className="mt-3 max-w-sm rounded-lg border border-primary/30 bg-primary/10 p-3 text-center text-xs leading-5 text-accent-foreground"
              >
                Há uma entrada de outro dia sem saída. Confira antes de encerrar: a
            ...
            <Button
              onClick={handleClick}
              disabled={isPending || disabled || !!blockedMessage}
              size="lg"
              className="mt-5 h-11 w-full max-w-60 rounded-full bg-primary font-semibold text-primary-foreground shadow-sm hover:brightness-105 motion-reduce:transition-none dark:bg-linear-to-r dark:from-primary dark:to-[#ffc477]"
            >
              {isPending ? (
            ...
            {blockedMessage && (
              <p
                role="status"
                className="mt-3 text-center text-sm text-accent-foreground"
              >
                {blockedMessage}
              </p>
            )}
        ...
        <p className="text-center text-xs text-muted-foreground xl:hidden">
          {dateLabel ? `${dateLabel} · ` : ""}América/São Paulo (BRT)
        </p>
      </CardContent>
    </Card>
  );
